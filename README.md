...
识图模型支持调用任意兼容 OpenAI API 的多模态模型，不限定为小米 MiMo（MiMo 仅作为默认示例）。
...
<p align="center">
  <img src="./assets/images/kaijutuoersuo_gameplay_screenshot.jpg" alt="开局托儿所-游戏界面" width="620" />
</p>

UI 截图：

<p>
  <img src="./assets/images/kaijutuoersuo_ui_screenshot.png" alt="开局托儿所-UI截图" width="300" />
  <img src="./assets/images/kaijutuoersuo_board_sample.png" alt="开局托儿所-棋盘样例" width="300" />
</p>
...
- `MIMO_API_KEY`：当前配置服务商的 key（默认示例是 MiMo）
- `OPENAI_API_KEY`：仅 `--llm-provider openai` 时需要
- `MIMO_VISION_MODEL`：模型识别图片时用的视觉模型（默认 `mimo-v2.5`，可替换成其他多模态模型名）
...
说明：该模式由“你配置的多模态识图模型”负责识别数字，本地算法负责找和为10的最优框选，稳定性更高。
...
1. 求解策略升级
- 多步规划：一次生成多步非重叠解链，减少截图/识别频次。
- 贪心 + 回溯：不仅看当前一步最优，还评估后续若干步收益，提升总分稳定性。
- 启发式剪枝：在非负数条件下，当局部和已 `>10` 时提前终止扩展，减少无效遍历。

2. 模型能力增强
- 多模型切换：同一套流程支持不同多模态模型，比较识别稳定性与延迟。
- 模型候选校验：模型返回候选后统一走本地规则校验，过滤非法解并重排序。

3. 稳定性与可观测性
- 重试与降级：网络超时时自动重试，失败后回退缓存或本地 OCR。
- 运行日志分级：增加关键指标（识别耗时、求解耗时、成功率、累计得分）便于调参。
